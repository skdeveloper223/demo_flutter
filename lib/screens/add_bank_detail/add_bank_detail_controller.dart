  ...
  addBankAccount() async {
    ...
    if (imageFile.isNotEmpty) {
      formData.files.add(
        MapEntry(
          'front_image',
          await MultipartFile.fromFile(imageFile.value, contentType: MediaType('image', 'png'), filename: imageFile.value.split('/').last),
        ),
      );
    }
    if (imageFile1.isNotEmpty) {
      formData.files.add(
        MapEntry(
          'back_image',
          await MultipartFile.fromFile(imageFile1.value, contentType: MediaType('image', 'png'), filename: imageFile1.value.split('/').last),
        ),
      );
    }

    final data = await APIFunction()
        .apiCall(apiName: Constants.addBankAccount, context: Get.context!, params: formData, token: getStorageData.readObject(getStorageData.token));
